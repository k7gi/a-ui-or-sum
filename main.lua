...
-- // cleanups
if game.CoreGui:FindFirstChild("Kagi") then game.CoreGui:FindFirstChild("Kagi"):Destroy() end 
...
local function CreateWatermark(title)
	local MainFrame = Instance.new("ScreenGui")
	MainFrame.Name = "WatermarkUI"
	MainFrame.Parent = game:GetService("CoreGui")
	title = title or "KAGI HUB"
	...
	vh_title.Position = UDim2.new(0.091, 0, 0.107, 0)
	vh_title.Size = UDim2.new(0, 66, 0, 25)
	vh_title.Font = Enum.Font.GothamBold
	vh_title.Text = title or "KAGI HUB"
	...
	glow.Size = UDim2.new(0.995, 30, 0.939, 30)
	glow.ZIndex = 0
	...
	vhlogo.Position = UDim2.new(0.046, 0, 0.076, 3)
	...
	clock.Position = UDim2.new(0.277, 0, 0.107, 0)
	...
	fps.Position = UDim2.new(0.395, 0, 0.107, 0)
	...
	ping.Position = UDim2.new(0.550, 0, 0.107, 0)
	...
	username.Position = UDim2.new(0.80, 0, 0.107, 0)
	...
	task.spawn(function()
		...
		while task.wait(0.1) do
			local result = os.date("%H:%M", os.time())
			local theping = string.split(string.split(Stats.Network.ServerStatsItem["Data Ping"]:GetValueString(), " ")[1], ".")
			local thefps = string.split(Stats.Workspace.Heartbeat:GetValueString(), ".")
...
local Kagi = Instance.new("ScreenGui")
--ProtectGui(Kagi);
Kagi.Name = "Kagi"
Kagi.Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or CoreGui
Kagi.ZIndexBehavior = Enum.ZIndexBehavior.Global
...
local Main = lib:Create('<font color="rgb(107, 89, 222)">KAGI HUB</font>',  UDim2.new(0, 700, 0, 600), Enum.KeyCode.RightShift)
...
do
    ...
    _G.__LockedClock = _G.__LockedClock
    WorldTab:Toggle('Left','Hook time', false, function(vvv)
        if _G.__HourLockConn then _G.__HourLockConn:Disconnect(); _G.__HourLockConn=nil end
        if vvv then  
...
WorldTab:Button('Left','Secret Button',function() print("Button clicked") end)
